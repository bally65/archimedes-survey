...
# 前端蓋（z=0 外側，朝負Z）
all_tris += make_ring(PIPE_R, ROD_R+2, -CAP_T, 0)

# 後端蓋（z=PIPE_LEN 外側，朝正Z）
all_tris += make_ring(PIPE_R, ROD_R+2, PIPE_LEN, PIPE_LEN+CAP_T)
...
# 端蓋2 + 中心軸2
all_tris += shift_y(make_ring(PIPE_R, ROD_R+2, -CAP_T, 0), DY2)
all_tris += shift_y(make_ring(PIPE_R, ROD_R+2, PIPE_LEN, PIPE_LEN+CAP_T), DY2)
