...
def make_hollow_rect(W, H, L, wall, z0=0.0):
    ...
    t = []
    hw, hh = W/2, H/2
    iw, ih = (W-2*wall)/2, (H-2*wall)/2
    # outer walls
    t += make_box(-hh, hh, -hw, hw, z0, z0+L)
    # punch inner (approx by 4 thin wall slabs)
    # Bottom wall
    t += make_box(-hh, -ih, -hw, hw, z0+wall, z0+L-wall)
    # Top wall
    t += make_box( ih,  hh, -hw, hw, z0+wall, z0+L-wall)
    # Left wall
    t += make_box(-ih, ih, -hw, -iw, z0+wall, z0+L-wall)
    # Right wall
    t += make_box(-ih, ih,  iw,  hw, z0+wall, z0+L-wall)
    # End caps (solid ends)
    t += make_box(-hh, hh, -hw, hw, z0,       z0+wall)
    t += make_box(-hh, hh, -hw, hw, z0+L-wall, z0+L)
    return t
...
def make_gusset(lx, lz, thickness=3, mirror_y=False):
    ...
    if mirror_y:
        t = [([v[0],-v[1],v[2]] for v in tri) for tri in t]
    return t
