...
def make_blade_segment(seg_idx, offset_deg=0.0):
    ...
    # 葉片上表面 & 下表面（內→外，4個角）
    def blade_face(r_in, r_out, ang, z_ang, dz):
        """在角度 ang，高度 z_ang+dz 處，生成內外兩個頂點"""
        return (
            [r_in  * np.cos(ang), r_in  * np.sin(ang), z_ang + dz],
            [r_out * np.cos(ang), r_out * np.sin(ang), z_ang + dz],
        )

    # 上下表面各四角（inner/outer × start/end）
    top_i0, top_o0 = blade_face(INNER_R, OUTER_R, a0, z0, +half_t)
    top_i1, top_o1 = blade_face(INNER_R, OUTER_R, a1, z1, +half_t)
    bot_i0, bot_o0 = blade_face(INNER_R, OUTER_R, a0, z0, -half_t)
    bot_i1, bot_o1 = blade_face(INNER_R, OUTER_R, a1, z1, -half_t)
